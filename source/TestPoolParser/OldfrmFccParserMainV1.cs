...
namespace TestPoolParser
{
	...
	public partial class OldfrmFccParserMainV1 : Form
	{
		//******************* global vars **************************
		...
		private void frmParserMain_Shown(object sender, EventArgs e)
		{
			...
			rbOutput2DB.Enabled = true;
			rbOutput2DB.Checked = Properties.Settings.Default.OutputToDB;
			...
			txtQuestionFileName.Text = Properties.Settings.Default.OutputQFileName;
			txtDescriptionFileName.Text = Properties.Settings.Default.OutputDFileName;
			if (rbOutput2DB.Checked)
			{
				txtOutputFolder.Enabled = false;
				txtDescriptionFileName.Enabled = false;
				txtQuestionFileName.Enabled = false;
			}
		...
		private void btnRun_Click(object sender, EventArgs e)
		{
			...
			if (udElementNumber.Value < 2 || udElementNumber.Value > 4)
			{
			...
			if (iElementNumber != (int)udElementNumber.Value)
			{
				iElementNumber = (int)udElementNumber.Value;
			...
			else // text option is chosen
			{
				if (!ValidateTextFileReady())
				{
					txtStatus.Text = "Parse Cancelled";
					MessageBox.Show(this, "Parse is cancelled.", "Text File Error");
					return;
				}
			}
			...
			try
			{
				oDocParser.ElementNumber = (int)udElementNumber.Value;
		...
		private void rbOutput2DB_CheckedChanged(object sender, EventArgs e)
		{
			Properties.Settings.Default.OutputToDB = rbOutput2DB.Checked;
			Properties.Settings.Default.Save();
			if (rbOutput2DB.Checked)
			{
				txtOutputFolder.Enabled = false;
				txtDescriptionFileName.Enabled = false;
				txtQuestionFileName.Enabled = false;
				btnFindOutputFolder.Enabled = false;
			...
			else
			{
				...
				txtDescriptionFileName.Enabled = true;
				txtQuestionFileName.Enabled = true;
				btnFindOutputFolder.Enabled = true;
				txtDBname.Enabled = false;
				btnSelectDB.Enabled = false;
			}
		}

		private void btnFindQuestFile_Click(object sender, EventArgs e)
		{
			FindQuestionFile();
		}

		private void btnFindDescFile_Click(object sender, EventArgs e)
		{
			FindDescriptionFile();
		}
		...
		private void SaveQuestionsToMySQL()
		{
			...
			oSQLiteHandler.ElementNumber = (int)udElementNumber.Value;
			oSQLiteHandler.DBpath = sDBfilepath;
			//-------- now do it
		...
		private bool LoadSourceFile(string FullPath)
		{
			...
			if (bInitializing)
			{
				...
				{
					udElementNumber.Value = iTestVal;
					Application.DoEvents();
				}
			}
			else
			{
				udElementNumber.Value = 0;
				Application.DoEvents();
		...
		{
			oTextHandler.SaveQuestionsToFile((int)udElementNumber.Value, QuestionsCommandList, DescriptionsCommandList);
		}

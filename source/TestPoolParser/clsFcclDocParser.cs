...
namespace TestPoolParser
{
    public class clsFcclDocParser
    {
        ...
        readonly Regex oRegxGheader = new Regex(@"^ ?[A-Z][0-9][0-9]?[A-Z] ");
                ...
                while (aLine != null)
                {
                    ...
                    if (aLine != "")
                    {
                        ...
                        if (aLine.Length >= 9) //---> test questions?
                        {
                            ...
                            if ((oRegxQheader.Match(aLine)).Success)
                            {
                                sGraphicFileName = "";
                            ...
                            {
                                if ((oRegxGheader.Match(aLine)).Success)
                                {
                                    sGroup = aLine.Substring(0, 3);
                                    if (sGroup.StartsWith("E6"))
                                    {
                                        Debug.WriteLine("found " + sGroup);
                                    }
                                    CreateDescriptionsCommands(aLine);
                                }
                            }
